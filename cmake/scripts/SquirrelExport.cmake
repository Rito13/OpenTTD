...
foreach(LINE IN LISTS SOURCE_LINES)
    ...
    if("${LINE}" MATCHES "^([\t ]*)\\* @api (.*)$")
        ...
        elseif("${LINE}" MATCHES "-${APILC}")
            set(API_SELECTED FALSE)
        elseif("${LINE}" MATCHES "${APILC}\\.([a-zA-Z]*)")
            set(SUB_API "${CMAKE_MATCH_1}")
        elseif("${LINE}" MATCHES "${APILC}")
            set(API_SELECTED TRUE)
        endif()
        continue()
    ...
    if("${LINE}" MATCHES "^([ \t]*)$")
        ...
        string(APPEND SQUIRREL_EXPORT "\nvoid SQ${API_CLS}_Register(Squirrel &engine, [[maybe_unused]] ${APIUC}SubAPIs sub_apis)")
        string(APPEND SQUIRREL_EXPORT "\n{")
        ...
        foreach(STATIC_METHOD IN LISTS STATIC_METHODS)
            ...
            list(GET STATIC_METHOD 3 SUB_API_IF)
            if(SUB_API_IF)
                set(SUB_API_IF "if (sub_apis.Test(${APIUC}SubAPI::${SUB_API_IF})) ")
            endif()
            if(SUSPENDABLE)
                set(SUSPENDABLE ", true")
            endif()
            if("${TYPES}" STREQUAL "v")
                string(APPEND SQUIRREL_EXPORT "\n\t${SUB_API_IF}SQ${API_CLS}.DefSQAdvancedStaticMethod(engine, &${CLS}::${FUNCNAME}, \"${FUNCNAME}\"${SUSPENDABLE});")
            else()
                string(APPEND SQUIRREL_EXPORT "\n\t${SUB_API_IF}SQ${API_CLS}.DefSQStaticMethod(engine, &${CLS}::${FUNCNAME}, \"${FUNCNAME}\", \"${TYPES}\"${SUSPENDABLE});")
            endif()
        endforeach()
        ...
        foreach(METHOD IN LISTS METHODS)
            ...
            if("${TYPES}" STREQUAL "v")
                string(APPEND SQUIRREL_EXPORT "\n\t${SUB_API_IF}SQ${API_CLS}.DefSQAdvancedMethod(engine, &${CLS}::${FUNCNAME}, \"${FUNCNAME}\"${SUSPENDABLE});")
            else()
                string(APPEND SQUIRREL_EXPORT "\n\t${SUB_API_IF}SQ${API_CLS}.DefSQMethod(engine, &${CLS}::${FUNCNAME}, \"${FUNCNAME}\", \"${TYPES}\"${SUSPENDABLE});")
            endif()
        endforeach()
    ...
    if("${LINE}" MATCHES "^.*\\(.*\\).*$")
        ...
        elseif(IS_STATIC)
            list(APPEND STATIC_METHODS "${FUNCNAME}:${TYPES}:${SUSPENDABLE}:${SUB_API}")
        else()
            list(APPEND METHODS "${FUNCNAME}:${TYPES}:${SUSPENDABLE}:${SUB_API}")
        endif()
        unset(SUSPENDABLE)
        set(SUB_API "")
        continue()

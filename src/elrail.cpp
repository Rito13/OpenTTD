...
static void DrawRailCatenaryRailway(const TileInfo *ti)
{
	...
	for (DiagDirection i = DiagDirection::Begin; i < DiagDirection::End; i++) {
		SpriteID pylon_base = (halftile_corner != Corner::Invalid && HasBit(InclinedSlope(i), to_underlying(halftile_corner))) ? pylon_halftile : pylon_normal;
	...
	SpriteID wire_halftile = (halftile_corner != Corner::Invalid) ? GetWireBase(ti->tile, TCX_UPPER_HALFTILE) : wire_normal;

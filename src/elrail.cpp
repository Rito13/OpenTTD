 ...
 */
static inline SpriteID GetWireBase(TileIndex index, const Tile &tile, TileContext context = TCX_NORMAL)
{
 ...
 */
static inline SpriteID GetPylonBase(TileIndex index, const Tile &tile, TileContext context = TCX_NORMAL)
{
	const RailTypeInfo *rti = GetRailTypeInfo(GetRailType(tile));
...
static void DrawRailCatenaryRailway(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	for (DiagDirection i = DiagDirection::Begin; i < DiagDirection::End; i++) {
		...
		if (ppp_allowed[i].Any() && pcp_status.Test(i) && !override_pcp.Test(i) &&
			...
			const auto &ppp_orders = _ppp_order[i][GetTileLocationGroup(ti->index)];
			for (Direction k = Direction::Begin; k < Direction::End; k++) {
				...
				if (ppp_allowed[i].Test(temp)) {
					...
					AddSortableSpriteToDraw(pylon_base + _pylon_sprites[temp], PAL_NONE, x, y, elevation,
						{{-1, -1, 0}, {1, 1, BB_HEIGHT_UNDER_BRIDGE}, {1, 1, 0}}, IsTransparencySet(TransparencyOption::Catenary));

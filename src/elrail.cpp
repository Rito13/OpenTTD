...
/**
 * Get the electrified rail bits of a single (associated) sub-tile.
 * @param t tile index to check
 * @param rail tile to check
...
{
	if (auto iter = RailTileIterator::Iterate(t); !iter.empty()) {
 ...
 */
static inline SpriteID GetWireBase(TileIndex index, Tile tile, TileContext context = TCX_NORMAL)
{
	const RailTypeInfo *rti = GetRailTypeInfo(GetRailType(tile));
	SpriteID wires = GetCustomRailSprite(rti, index, RTSG_WIRES, context);
 ...
 */
static inline SpriteID GetPylonBase(TileIndex index, Tile tile, TileContext context = TCX_NORMAL)
{
	const RailTypeInfo *rti = GetRailTypeInfo(GetRailType(tile));
	SpriteID pylons = GetCustomRailSprite(rti, index, RTSG_PYLONS, context);
...
void DrawRailCatenaryOnTunnel(const TileInfo *ti)
{
	...
	AddSortableSpriteToDraw(
		...
		GetTilePixelZ(ti->index) + sss->z_offset,
...
static void DrawRailCatenaryRailway(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	trackconfig[TS_HOME] = GetSingleRailTrackBitsUniversal(ti->index, ti->tile, &override_pcp);
	if (IsValidCorner(halftile_corner)) trackconfig[TS_HOME] &= draw_halftile ? CornerToTrackBits(halftile_corner) : ~CornerToTrackBits(halftile_corner);
	wireconfig[TS_HOME] = MaskWireBits(ti->index, trackconfig[TS_HOME]);
	/* If a track bit is present that is not in the main direction, the track is level */
	isflat[TS_HOME] = ((trackconfig[TS_HOME] & (TRACK_BIT_HORZ | TRACK_BIT_VERT)) != 0);

	AdjustTileh(ti->index, &tileh[TS_HOME]);

	SpriteID pylon_base = GetPylonBase(ti->index, ti->tile, draw_halftile ? TCX_UPPER_HALFTILE : TCX_NORMAL);

	for (DiagDirection i = DIAGDIR_BEGIN; i < DIAGDIR_END; i++) {
		TileIndex neighbour = ti->index + TileOffsByDiagDir(i);
		int elevation = GetPCPElevation(ti->index, i);

		tileh[TS_NEIGHBOUR] = std::get<0>(GetFoundationSlope(neighbour));
		...
		if (ppp_allowed[i] != 0 && HasBit(pcp_status, i) && !HasBit(override_pcp, i) &&
			...
			for (Direction k = DIR_BEGIN; k < DIR_END; k++) {
				uint8_t temp = PPPorder[i][GetTLG(ti->index)][k];
	...
	SpriteID wire_base = GetWireBase(ti->index, ti->tile, draw_halftile ? TCX_UPPER_HALFTILE : TCX_NORMAL);
...
{
	TileIndex end = GetSouthernBridgeEnd(ti->index);
	...
	if (GetTunnelBridgeLength(ti->index, start) + 1 == length) {
		...
		case MP_RAILWAY:
			if (IsRailDepot(ti->tile)) {
				...
				AddSortableSpriteToDraw(
					...
					GetTileMaxPixelZ(ti->index) + sss->z_offset,
					IsTransparencySet(TO_CATENARY)
				);
				return;
			}

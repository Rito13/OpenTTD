...
{
	return s.Test(BaseSlope::HalfTile);
}
...
{
	return (s == BaseSlope::W) || (s == BaseSlope::S) || (s == BaseSlope::E) || (s == BaseSlope::N);
}
...
{
	return Corner(to_underlying(corner) ^ 2);
}
...
{
	return SlopeWithThreeCornersRaised(OppositeCorner(corner)).Set(BaseSlope::Steep);
}
...
{
	assert(IsValidCorner(corner));
	return static_cast<Slope>(s.Set(BaseSlope::HalfTile).Set(static_cast<BaseSlope>(to_underlying(corner) << 6)));
}

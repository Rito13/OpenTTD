 ...
 */
static constexpr TypedIndexContainer<std::array<Directions, 15>, Slope> _flood_from_dirs = {
	Directions{DIR_NW, DIR_SW, DIR_SE, DIR_NE}, // SLOPE_FLAT
	Directions{DIR_NE, DIR_SE}, // SLOPE_W
	Directions{DIR_NW, DIR_NE}, // SLOPE_S
	Directions{DIR_NE}, // SLOPE_SW
	Directions{DIR_NW, DIR_SW}, // SLOPE_E
	Directions{}, // SLOPE_EW
	Directions{DIR_NW}, // SLOPE_SE
	Directions{DIR_N, DIR_NW, DIR_NE}, // SLOPE_WSE, SLOPE_STEEP_S
	Directions{DIR_SW, DIR_SE}, // SLOPE_N
	Directions{DIR_SE}, // SLOPE_NW
	Directions{}, // SLOPE_NS
	Directions{DIR_E, DIR_NE, DIR_SE}, // SLOPE_NWS, SLOPE_STEEP_W
	Directions{DIR_SW}, // SLOPE_NE
	Directions{DIR_S, DIR_SW, DIR_SE}, // SLOPE_ENW, SLOPE_STEEP_N
	Directions{DIR_W, DIR_SW, DIR_NW}, // SLOPE_SEN, SLOPE_STEEP_E
};
		...
		case TileType::Water:
			switch (GetWaterTileType(tile)) {
				...
				case WaterTileType::Coast:
					...
					if (slope.Test(BaseSlope::W)) return (from == DIR_SE) || (from == DIR_E) || (from == DIR_NE);
					if (slope.Test(BaseSlope::S)) return (from == DIR_NE) || (from == DIR_N) || (from == DIR_NW);
					if (slope.Test(BaseSlope::E)) return (from == DIR_NW) || (from == DIR_W) || (from == DIR_SW);
					if (slope.Test(BaseSlope::N)) return (from == DIR_SW) || (from == DIR_S) || (from == DIR_SE);
					return false;
			}
	...
	switch (GetFloodingBehaviour(tile)) {
		case FLOOD_ACTIVE: {
			...
			for (Direction dir = DIR_BEGIN; dir < DIR_END; dir++) {
				...
				if (!_flood_from_dirs[Slope(slope_dest).Reset(SLOPE_HALFTILE_MASK).Reset(BaseSlope::Steep)].Test(ReverseDir(dir))) continue;
		...
		case FLOOD_DRYUP: {
			Slope slope_here = std::get<0>(GetFoundationSlope(tile)).Reset(SLOPE_HALFTILE_MASK).Reset(BaseSlope::Steep);
	...
	for (const auto tile : Map::Iterate()) {
		...
		if (IsTileType(tile, TileType::Clear) && z == 0) {
			...
			switch (slope.Count()) {
				...
				case 1:
					if (!slope.Any({BaseSlope::Steep, BaseSlope::HalfTile})) {
						MakeShore(tile);
						break;
					}
					[[fallthrough]];
				...
				default:
					for (Direction dir : _flood_from_dirs[slope.Reset(BaseSlope::Steep)]) {
						TileIndex dest = TileAddByDir(tile, dir);
						Slope slope_dest = GetTileSlope(dest).Reset(BaseSlope::Steep);
...
{
	static constexpr TypedIndexContainer<std::array<TrackBits, 16>, Slope> coast_tracks = {
		TRACK_BIT_NONE, TRACK_BIT_RIGHT, TRACK_BIT_UPPER, TRACK_BIT_NONE, TRACK_BIT_LEFT, TRACK_BIT_NONE, TRACK_BIT_NONE, TRACK_BIT_NONE,
		TRACK_BIT_LOWER, TRACK_BIT_NONE, TRACK_BIT_NONE, TRACK_BIT_NONE, TRACK_BIT_NONE, TRACK_BIT_NONE, TRACK_BIT_NONE, TRACK_BIT_NONE
	};

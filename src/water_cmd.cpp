	...
	if (tileh != SLOPE_FLAT) {
		...
		if (Tile::HasType(target, MP_RAILWAY)) {
			...
			if (!IsSlopeWithOneCornerRaised(tileh) && Tile::HasType(target, MP_TREES)) {
				...
				flooded = true;
			} else if (Command<CMD_LANDSCAPE_CLEAR>::Do(DoCommandFlag::Execute, target).Succeeded()) {
				MakeShore(target);
				MarkTileDirtyByTile(target);
				flooded = true;
			}
...
static TrackStatus GetTileTrackStatus_Water(TileIndex index, Tile tile, TransportType mode, uint, DiagDirection)
{
	...
	switch (GetWaterTileType(tile)) {
		case WATER_TILE_CLEAR: ts = IsTileFlat(index) ? TRACK_BIT_ALL : TRACK_BIT_NONE; break;
		case WATER_TILE_COAST: ts = coast_tracks[GetTileSlope(index) & 0xF]; break;
...
static bool ChangeTileOwner_Water(TileIndex index, Tile &tile, Owner old_owner, Owner new_owner)
{
	...
	/* Remove depot */
	if (IsShipDepot(tile)) Command<CMD_LANDSCAPE_CLEAR>::Do({DoCommandFlag::Execute, DoCommandFlag::Bankrupt}, index);
...
static VehicleEnterTileStates VehicleEnter_Water(Vehicle *, TileIndex, Tile, int, int)
{
	return {};
}

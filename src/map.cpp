...
/* static */ std::unique_ptr<Tile::BaseTile[]> Tile::base_tiles; ///< Base tiles of the map
...
/* static */ void Map::Allocate(uint size_x, uint size_y)
{
	...
	Tile::base_tiles = std::make_unique<Tile::BaseTile[]>(Map::size);
	...
	for (Tile t : Map::Iterate()) {
		t.SetOffset(TileIndex(t).base() % INDEXES_PER_SUB_TILES_CHUNK);
	}

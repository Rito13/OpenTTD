...
/* virtual */ uint32_t CanalScopeResolver::GetRandomBits() const
{
	...
	return IsMainTileType(this->tile, MP_WATER) ? GetWaterTileRandomBits(this->tile) : 0;
}
...
/* virtual */ uint32_t CanalScopeResolver::GetVariable(uint8_t variable, [[maybe_unused]] uint32_t parameter, bool &available) const
{
	switch (variable) {
		...
		case 0x80: {
			...
			if (IsMainTileType(this->tile, MP_WATER) && IsLock(this->tile) && GetLockPart(this->tile) == LOCK_PART_UPPER) z--;
			return z;
		}
		...
		case 0x83: return IsMainTileType(this->tile, MP_WATER) ? GetWaterTileRandomBits(this->tile) : 0;
	}

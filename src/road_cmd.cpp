...
template<RoadTramType T_RTT>
static bool (*_roadtypes_sort_functions[]) (const RoadType&, const RoadType&) = {
	nullptr, // RoadTypes are always sorted by sorting_order. No need to sort them twice.
	CompareRoadTypesByCost<T_RTT>,
	CompareRoadTypesBySpeed<T_RTT>,
	CompareRoadTypesByMaintenanceCost<T_RTT>,
	CompareRoadTypesByName<T_RTT>
};
...
template<RoadTramType T_RTT>
static const std::array<StringID, sizeof(_roadtypes_sort_functions<T_RTT>) / sizeof(_roadtypes_sort_functions<T_RTT>[0])> ROADTYPE_SORT_LISTING = {
	T_RTT == RTT_ROAD ? STR_SORT_BY_ROADTYPE_ID : STR_SORT_BY_TRAMTYPE_ID,
	STR_SORT_BY_COST,
...
{
	DropDownList list;
	for (uint8_t i = 0; i < ROADTYPE_SORT_LISTING<T_RTT>.size(); ++i) {
		list.push_back(MakeDropDownListStringItem(ROADTYPE_SORT_LISTING<T_RTT>[i], i));
	}
	return list;
}

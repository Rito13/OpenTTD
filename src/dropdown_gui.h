...
class RoadTypeDropdownWindowBase : public DropdownWindow {
public:
	/** @see DropdownWindow::DropdownWindow. */
	RoadTypeDropdownWindowBase(int window_id, Window *parent, DropDownList &&list, int selected, WidgetID button, const Rect wi_rect, bool instant_close, Colours wi_colour, bool persist) : DropdownWindow(window_id, parent, std::move(list), selected, button, wi_rect, instant_close, wi_colour, persist, true)
	{
		this->FinishInitNested(window_id);
		this->flags.Reset(WindowFlag::WhiteBorder);
	}
...
class TramTypeDropdownWindowBase : public DropdownWindow {
public:
	/** @see DropdownWindow::DropdownWindow. */
	TramTypeDropdownWindowBase(int window_id, Window *parent, DropDownList &&list, int selected, WidgetID button, const Rect wi_rect, bool instant_close, Colours wi_colour, bool persist) : DropdownWindow(window_id, parent, std::move(list), selected, button, wi_rect, instant_close, wi_colour, persist, true)
	{
		this->FinishInitNested(window_id);
		this->flags.Reset(WindowFlag::WhiteBorder);
	}
 ...
 * @tparam T_DropdownWindow Type to check.
 */
template <typename T_DropdownWindow>
concept DropdownWindowType = std::derived_from<T_DropdownWindow, DropdownWindow> && requires (int window_id, Window *parent, DropDownList &&list, int selected, WidgetID button, const Rect wi_rect, bool instant_close, Colours wi_colour, bool persist) {
	{ T_DropdownWindow(window_id, parent, std::move(list), selected, button, wi_rect, instant_close, wi_colour, persist) };
};

/**
 * Show a dropdown list of a custom dropdown window class.
 * @tparam T_DropdownWindow Class for new dropdown.
 ...
 */
template <DropdownWindowType T_DropdownWindow>
void ShowCustomDropdownList(Window *w, DropDownList &&list, int selected, WidgetID button, uint width = 0, bool instant_close = false, bool persist = false)
{
	...
	new T_DropdownWindow(0, w, std::move(list), selected, button, wi_rect, instant_close, wi_colour, persist);
}
...
{
	T_DropdownWindow *ddw = dynamic_cast<T_DropdownWindow *>(parent->FindChildWindow(WC_DROPDOWN_MENU));

...
std::tuple<CommandCost, Money, TileIndex> CmdTerraformLand(DoCommandFlags flags, TileIndex tile, Slope slope, bool dir_up)
{
	...
	if (slope.Test(BaseSlope::W) && tile + TileDiffXY(1, 0) < Map::Size()) {
	...
	if (slope.Test(BaseSlope::S) && tile + TileDiffXY(1, 1) < Map::Size()) {
	...
	if (slope.Test(BaseSlope::E) && tile + TileDiffXY(0, 1) < Map::Size()) {
	...
	for (int pass = 0; pass < 2; pass++) {
		for (const auto &t : ts.dirty_tiles) {
			...
			/* Compute tile slope */
			Slope tileh = (z_max > z_min + 1 ? Slope{BaseSlope::Steep} : SLOPE_FLAT);
			if (z_W > z_min) tileh.Set(BaseSlope::W);
			if (z_S > z_min) tileh.Set(BaseSlope::S);
			if (z_E > z_min) tileh.Set(BaseSlope::E);
			if (z_N > z_min) tileh.Set(BaseSlope::N);
...
std::tuple<CommandCost, Money, TileIndex> CmdLevelLand(DoCommandFlags flags, TileIndex tile, TileIndex start_tile, bool diagonal, LevelMode lm)
{
	...
	for (; *iter != INVALID_TILE; ++(*iter)) {
		...
		while (curh != h) {
			CommandCost ret;
			std::tie(ret, std::ignore, error_tile) = Command<Commands::TerraformLand>::Do(DoCommandFlags{flags}.Reset(DoCommandFlag::Execute), t, BaseSlope::N, curh <= h);
			if (ret.Failed()) {
			...
			if (flags.Test(DoCommandFlag::Execute)) {
				...
				Command<Commands::TerraformLand>::Do(flags, t, BaseSlope::N, curh <= h);
			} else {

	...
	if (IsBridgeTile(tile_start) && IsBridgeTile(tile_end) &&
		...
		for (TileIndex tile = tile_start + delta; tile != tile_end; tile += delta) {
			...
			for (Tile rail : RailTileIterator::Iterate(tile)) {
				if (!IsNormalRail(rail)) goto not_valid_below;
			}
...
static void DrawTile_TunnelBridge(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (IsTunnel(ti->tile)) {
		...
		if (transport_type == TRANSPORT_ROAD) {
			...
			DrawBridgeRoadBits(ti->index, ti->x, ti->y, z, offset, true);

			EndSpriteCombine();
		} else if (transport_type == TRANSPORT_RAIL) {
			const RailTypeInfo *rti = GetRailTypeInfo(GetRailType(ti->tile));
			if (rti->UsesOverlay()) {
				SpriteID surface = GetCustomRailSprite(rti, ti->index, RTSG_BRIDGE);
				if (surface != 0) {
...
void DrawBridgeMiddle(const TileInfo *ti)
{
	...
	BridgePieces piece = CalcBridgePiece(
		GetTunnelBridgeLength(ti->index, rampnorth) + 1,
		GetTunnelBridgeLength(ti->index, rampsouth) + 1
	);
...
static TrackStatus GetTileTrackStatus_TunnelBridge(TileIndex, Tile tile, TransportType mode, uint sub_mode, DiagDirection side)
{
...
static bool ChangeTileOwner_TunnelBridge(TileIndex index, Tile &tile, Owner old_owner, Owner new_owner)
{
	...
	if (new_owner != INVALID_OWNER) {
		...
		if (tt == TRANSPORT_RAIL) {
			 ...
			 * bridge / tunnel. */
			[[maybe_unused]] CommandCost ret = Command<CMD_LANDSCAPE_CLEAR>::Do({DoCommandFlag::Execute, DoCommandFlag::Bankrupt}, index);
...
static VehicleEnterTileStates VehicleEnter_TunnelBridge(Vehicle *v, TileIndex index, Tile tile, int x, int y)
{
...
static CommandCost TerraformTile_TunnelBridge(TileIndex index, Tile tile, DoCommandFlags, int z_new, Slope tileh_new)
{

...
Foundation GetRailFoundation(Slope tileh, TrackBits bits)
{
	...
	if (IsSteepSlope(tileh)) {
		...
		switch (bits) {
			...
			case TRACK_BIT_HORZ:
				if (tileh == BaseSlope::N) return HalftileFoundation(Corner::N);
				if (tileh == BaseSlope::S) return HalftileFoundation(Corner::S);
				return (valid_on_leveled ? Foundation::Leveled : Foundation::Invalid);

			case TRACK_BIT_VERT:
				if (tileh == BaseSlope::W) return HalftileFoundation(Corner::W);
				if (tileh == BaseSlope::E) return HalftileFoundation(Corner::E);
				return (valid_on_leveled ? Foundation::Leveled : Foundation::Invalid);
...
static void DrawTrackFence_NW(const TileInfo *ti, const PalSpriteID &psid, uint num_sprites)
{
	...
	if (ti->tileh.Any(SLOPE_NW)) rfo = ti->tileh.Test(BaseSlope::W) ? RFO_SLOPE_SW_NW : RFO_SLOPE_NE_NW;
...
static void DrawTrackFence_SE(const TileInfo *ti, const PalSpriteID &psid, uint num_sprites)
{
	...
	if (ti->tileh.Any(SLOPE_SE)) rfo = ti->tileh.Test(BaseSlope::S) ? RFO_SLOPE_SW_SE : RFO_SLOPE_NE_SE;
	DrawTrackFence(ti, psid, num_sprites, rfo);
}
...
static void DrawTrackFence_NE(const TileInfo *ti, const PalSpriteID &psid, uint num_sprites)
{
	...
	if (ti->tileh.Any(SLOPE_NE)) rfo = ti->tileh.Test(BaseSlope::E) ? RFO_SLOPE_SE_NE : RFO_SLOPE_NW_NE;
	DrawTrackFence(ti, psid, num_sprites, rfo);
}
...
static void DrawTrackFence_SW(const TileInfo *ti, const PalSpriteID &psid, uint num_sprites)
{
	...
	if (ti->tileh.Any(SLOPE_SW)) rfo = ti->tileh.Test(BaseSlope::S) ? RFO_SLOPE_SE_SW : RFO_SLOPE_NW_SW;
	DrawTrackFence(ti, psid, num_sprites, rfo);
}
...
static void DrawTrackBitsOverlay(TileInfo *ti, TrackBits track, const RailTypeInfo *rti)
{
	...
	if (track == TRACK_BIT_NONE) {
		...
		if (pbs & TRACK_BIT_UPPER) DrawTrackSprite(overlay + RTO_N, PALETTE_CRASH, ti, BaseSlope::N);
		if (pbs & TRACK_BIT_LOWER) DrawTrackSprite(overlay + RTO_S, PALETTE_CRASH, ti, BaseSlope::S);
		if (pbs & TRACK_BIT_RIGHT) DrawTrackSprite(overlay + RTO_E, PALETTE_CRASH, ti, BaseSlope::E);
		if (pbs & TRACK_BIT_LEFT)  DrawTrackSprite(overlay + RTO_W, PALETTE_CRASH, ti, BaseSlope::W);
		...
		switch (track) {
			...
			case TRACK_BIT_UPPER: DrawTrackSprite(ground + RTO_N, PAL_NONE, ti, BaseSlope::N); break;
			case TRACK_BIT_LOWER: DrawTrackSprite(ground + RTO_S, PAL_NONE, ti, BaseSlope::S); break;
			case TRACK_BIT_RIGHT: DrawTrackSprite(ground + RTO_E, PAL_NONE, ti, BaseSlope::E); break;
			case TRACK_BIT_LEFT:  DrawTrackSprite(ground + RTO_W, PAL_NONE, ti, BaseSlope::W); break;
			case TRACK_BIT_CROSS: DrawGroundSprite(ground + RTO_CROSSING_XY, PAL_NONE); break;
			case TRACK_BIT_HORZ:  DrawTrackSprite(ground + RTO_N, PAL_NONE, ti, BaseSlope::N);
			                      DrawTrackSprite(ground + RTO_S, PAL_NONE, ti, BaseSlope::S); break;
			case TRACK_BIT_VERT:  DrawTrackSprite(ground + RTO_E, PAL_NONE, ti, BaseSlope::E);
			                      DrawTrackSprite(ground + RTO_W, PAL_NONE, ti, BaseSlope::W); break;
...
static void DrawTrackBits(TileInfo *ti, TrackBits track)
{
	...
	if (_game_mode != GM_MENU && _settings_client.gui.show_track_reservation) {
		...
		if (pbs & TRACK_BIT_UPPER) DrawGroundSprite(rti->base_sprites.single_n, PALETTE_CRASH, nullptr, 0, ti->tileh.Test(BaseSlope::N) ? -static_cast<int>(TILE_HEIGHT) : 0);
		if (pbs & TRACK_BIT_LOWER) DrawGroundSprite(rti->base_sprites.single_s, PALETTE_CRASH, nullptr, 0, ti->tileh.Test(BaseSlope::S) ? -static_cast<int>(TILE_HEIGHT) : 0);
		if (pbs & TRACK_BIT_LEFT)  DrawGroundSprite(rti->base_sprites.single_w, PALETTE_CRASH, nullptr, 0, ti->tileh.Test(BaseSlope::W) ? -static_cast<int>(TILE_HEIGHT) : 0);
		if (pbs & TRACK_BIT_RIGHT) DrawGroundSprite(rti->base_sprites.single_e, PALETTE_CRASH, nullptr, 0, ti->tileh.Test(BaseSlope::E) ? -static_cast<int>(TILE_HEIGHT) : 0);
	}

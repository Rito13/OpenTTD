...
static CommandCost CheckTrackCombination(TileIndex tile, TrackBits to_build)
{
	...
	if (IsLevelCrossingTile(tile)) return CommandCost(STR_ERROR_MUST_REMOVE_ROAD_FIRST);
...
static CommandCost CheckRailTiles(TileIndex tile, Track track, RailType railtype, bool auto_remove_signals, DoCommandFlags flags)
{
	...
	for (Tile rail : RailTileIterator::Iterate(tile)) {
		if (!IsPlainRail(rail)) return Command<CMD_LANDSCAPE_CLEAR>::Do(flags, tile); // just get appropriate error message
	...
	if (need_convert) {
		CommandCost ret = Command<CMD_CONVERT_RAIL>::Do(flags, tile, tile, railtype, false);
		if (ret.Failed()) return ret;
...
CommandCost CmdBuildSingleRail(DoCommandFlags flags, TileIndex tile, RailType railtype, Track track, bool auto_remove_signals)
{
	...
	if (Tile::HasType(tile, MP_RAILWAY)) {
		...
		if (flags.Test(DoCommandFlag::Execute)) {
			...
			Tile rail{};
			for (Tile cur_tile : RailTileIterator::Iterate(tile)) {
				/* Check if the track overlaps the current rail tile. */
				TrackBits tracks = GetTrackBits(cur_tile);
				if (TrackOverlapsTracks(tracks, track)) {
					rail = cur_tile;
					break;
				}

				/* Tracks don't overlap, but the track might still be combinable with the present track. */
				if (HasBit(tracks, TrackToOppositeTrack(track))) {
					if (IsTileOwner(cur_tile, _current_company) && GetRailType(cur_tile) == railtype) {
						rail = cur_tile;
						break;
					}
				}
			...
			if (rail.IsValid()) {
				/* Modify existing rail sub-tile. */
				TrackBits bits = GetTrackBits(rail);
				SetTrackBits(rail, bits | trackbit);
				SetRailFenceType(rail, RAIL_FENCE_NONE);
				MakeClearGrass(tile); // Make barren ground.
				...
				for (Tile t : RailTileIterator::Iterate(tile)) {
					SetRailFenceType(t, RAIL_FENCE_NONE);
				}
			}
		}
	} else {
		if (IsTileType(tile, MP_ROAD)) {
			...
			if (IsNormalRoad(tile)) {
				...
				if ((track == TRACK_X && ((road | tram) & ROAD_X) == 0) ||
					...
					if (flags.Test(DoCommandFlag::Execute)) {
						/* Make sure the road is a full tile, not a half-tile. */
						if (road != ROAD_NONE) SetRoadBits(tile, (track == TRACK_X ? ROAD_Y : ROAD_X), RTT_ROAD);
						if (tram != ROAD_NONE) SetRoadBits(tile, (track == TRACK_X ? ROAD_Y : ROAD_X), RTT_TRAM);
						SetRoadside(tile, ROADSIDE_BARREN);

						MakeLevelCrossing(tile, _current_company, TrackToTrackBits(track), railtype);
						UpdateLevelCrossing(tile, false);
						MarkDirtyAdjacentLevelCrossingTiles(tile, track == TRACK_X ? AXIS_Y : AXIS_X);
		...
		if (IsTileType(tile, MP_WATER) && IsSlopeWithOneCornerRaised(tileh)) {
			assert(!Tile::HasType(tile, MP_TREES));
...
/**
 * Remove a single piece of track from a rail tile.
 * @param flags operation to perform
 * @param tile tile index to remove track from
 * @param rail The rail sub-tile to remove the track from. Will be modified if the rail tile is removed.
 * @param track rail orientation
 * @return the cost of this operation or an error
 */
static CommandCost RemoveSingleRail(DoCommandFlags flags, TileIndex tile, Tile &rail, Track track)
{
	assert(IsValidTrack(track));
	assert(rail.IsValid());

    /* There are no rails present at depots. */
	if (!IsNormalRail(rail)) return CommandCost(STR_ERROR_THERE_IS_NO_RAILROAD_TRACK);
	...
	TrackBits present = GetTrackBits(rail);
	TrackBits trackbit = TrackToTrackBits(track);
	if ((present & trackbit) == 0) return CommandCost(STR_ERROR_THERE_IS_NO_RAILROAD_TRACK);
	bool crossing = present == (TRACK_BIT_X | TRACK_BIT_Y);

	CommandCost cost(EXPENSES_CONSTRUCTION, RailClearCost(GetRailType(rail)));

	/* Charge extra to remove signals on the track, if they are there */
	if (HasSignalOnTrack(rail, track)) {
		cost.AddCost(Command<CMD_REMOVE_SINGLE_SIGNAL>::Do(flags, tile, track));
	}

	if (flags.Test(DoCommandFlag::Execute)) {
		if (IsLevelCrossing(rail)) {
			UpdateAdjacentLevelCrossingTilesOnLevelCrossingRemoval(tile, track == TRACK_X ? AXIS_Y : AXIS_X);
		}

		Train *v = nullptr;
		if (HasRailReservationTrackBits(rail, trackbit)) {
		...
		Owner owner = GetTileOwner(rail);

		/* Subtract old infrastructure count. */
		uint pieces = CountBits(present);
		if (TracksOverlap(present)) pieces *= pieces;
		if (IsLevelCrossing(rail)) pieces *= LEVELCROSSING_TRACKBIT_FACTOR;
		Company::Get(owner)->infrastructure.rail[GetRailType(rail)] -= pieces;
		/* Add new infrastructure count. */
		present ^= trackbit;
		...
		if (present == 0) {
			rail = Tile::Remove(tile, rail);
			if (!IsTileType(tile, MP_ROAD) && (!IsTileType(tile, MP_WATER) || !IsSlopeWithOneCornerRaised(GetTileSlope(tile)))) {
				MakeClearGrass(tile);
			}
			if (!Tile::HasType(tile, MP_RAILWAY)) DeleteNewGRFInspectWindow(GSF_RAILTYPES, tile.base());
		} else {
...
{
	assert(IsPlainRailTile(t)); // If one associated tile is plain rail, all tiles will be, so no need to check more.
...
CommandCost CmdBuildTrainDepot(DoCommandFlags flags, TileIndex tile, RailType railtype, DiagDirection dir)
{
	...
	if (Tile rail = Tile::GetByType(tile, MP_RAILWAY); IsRailDepotTile(rail) && railtype == GetRailType(rail)) {
		CommandCost ret = CheckTileOwnership(tile, rail);
		if (ret.Failed()) return ret;
...
CommandCost CmdBuildSingleSignal(DoCommandFlags flags, TileIndex tile_index, Track track, SignalType sigtype, SignalVariant sigvar, bool convert_signal, bool skip_existing_signals, bool ctrl_pressed, SignalType cycle_start, SignalType cycle_stop, uint8_t num_dir_cycle, uint8_t signals_copy)
{
	...
	if (flags.Test(DoCommandFlag::Execute)) {
		...
		if (IsPbsSignal(sigtype)) {
			...
			SetSignalStates(tile, (GetSignalStates(tile) & ~mask) | ((HasBit(GetRailReservationTrackBits(tile), track) && EnsureNoVehicleOnGround(tile_index).Succeeded() ? UINT_MAX : 0) & mask));
		}
...
static bool TryMergeRailTile(TileIndex index, Tile &tile)
{
	...
	assert((GetTrackBits(prev_tile) & GetTrackBits(tile)) == TRACK_BIT_NONE);
	...
	if (HasSignals(tile)) {
		...
		SetSignalVariant(prev_tile, TRACK_LOWER, static_cast<SignalVariant>(GetSignalVariant(prev_tile, TRACK_LOWER) | GetSignalVariant(tile, TRACK_LOWER)));
		SetSignalVariant(prev_tile, TRACK_UPPER, static_cast<SignalVariant>(GetSignalVariant(prev_tile, TRACK_UPPER) | GetSignalVariant(tile, TRACK_UPPER)));
	}
...
CommandCost CmdConvertRail(DoCommandFlags flags, TileIndex tile_index, TileIndex area_start, RailType totype, bool diagonal)
{
	...
	for (; (tile_index = *iter) != INVALID_TILE; ++(*iter)) {
		Tile tile = Tile::GetByType(tile_index, MP_RAILWAY);
		...
		TrainList vehicles_affected;
		do {
			tt = GetTileType(tile); // Re-get when looping.

			/* Check for disallowed level crossings. */
			if (IsLevelCrossingTile(tile) && RailNoLevelCrossings(totype)) {
				error.MakeError(STR_ERROR_CROSSING_DISALLOWED_RAIL);
				goto exit_inner; // Exit inner do loop, no further rail tiles possible.
			}
			...
			if (type == totype || (_settings_game.vehicle.disable_elrails && totype == RAILTYPE_RAIL && type == RAILTYPE_ELECTRIC)) goto exit_inner;
			...
			if (tt != MP_TUNNELBRIDGE) {
				if (!IsCompatibleRail(type, totype)) {
					ret = IsNormalRailTile(tile) ? EnsureNoTrainOnTrackBits(tile_index, GetTrackBits(tile)) : EnsureNoVehicleOnGround(tile_index);
					if (ret.Failed()) {
						error = std::move(ret);
						goto exit_inner;
					}
				...
				if (flags.Test(DoCommandFlag::Execute)) { // we can safely convert, too
					TrackBits reserved = (tt == MP_RAILWAY) ? GetRailReservationTrackBits(tile) : GetReservedTrackbits(tile_index);
					Track     track;
			...
			switch (tt) {
					...
					switch (GetRailTileType(tile)) {
						case RAIL_TILE_DEPOT:
							if (flags.Test(DoCommandFlag::Execute)) {
								...
								InvalidateWindowData(WC_VEHICLE_DEPOT, tile_index);
								InvalidateWindowData(WC_BUILD_VEHICLE, tile_index);
							}
						...
						default: // RAIL_TILE_NORMAL, RAIL_TILE_SIGNALS, RAIL_TILE_CROSSING
							if (flags.Test(DoCommandFlag::Execute)) {
								...
								while (tracks != TRACK_BIT_NONE) {
									YapfNotifyTrackLayoutChange(tile_index, RemoveFirstTrack(&tracks));
								}
							}
							found_convertible_track = true;
				...
				case MP_TUNNELBRIDGE: {
					...
					if (endtile < tile_index) {
						if (diagonal) {
							if (DiagonalTileArea(area_start, area_end).Contains(endtile)) goto exit_inner;
						} else {
							if (OrthogonalTileArea(area_start, area_end).Contains(endtile)) goto exit_inner;
						}
					}
				...
				default: // MP_STATION
					if (flags.Test(DoCommandFlag::Execute)) YapfNotifyTrackLayoutChange(tile_index, GetRailStationTrack(tile));
			...
			if (!IsTileType(tile, MP_RAILWAY)) break;

			if (TryMergeRailTile(tile_index, tile)) { // Will modify tile if it is merged.
				if (!IsTileType(tile, MP_RAILWAY)) tile = tile.GetNextByType(MP_RAILWAY);
			} else {
				tile = tile.GetNextByType(MP_RAILWAY);
			}
		} while (tile.IsValid());
...
static std::tuple<CommandCost, bool> RemoveTrainDepot(TileIndex index, Tile &tile, DoCommandFlags flags)
{
	if (_current_company != OWNER_WATER) {
	...
	return {CommandCost(EXPENSES_CONSTRUCTION, _price[PR_CLEAR_DEPOT_TRAIN]), flags.Test(DoCommandFlag::Execute)};
}

static std::tuple<CommandCost, bool> ClearTile_Track(TileIndex index, Tile &tile, DoCommandFlags flags)
{
	CommandCost cost(EXPENSES_CONSTRUCTION);
	...
	switch (GetRailTileType(tile)) {
		...
		case RAIL_TILE_NORMAL: {
			...
			while (tracks != TRACK_BIT_NONE) {
				...
				CommandCost ret = RemoveSingleRail(flags, index, tile, track);
				if (ret.Failed()) return {ret, false};
				cost.AddCost(ret.GetCost());
			}

			return {cost, flags.Test(DoCommandFlag::Execute)};
		}

		case RAIL_TILE_DEPOT:
			return RemoveTrainDepot(index, tile, flags);
...
static void DrawSingleSignal(TileIndex index, Tile tile, const RailTypeInfo *rti, Track track, SignalState condition, SignalOffsets image, uint pos)
{
	bool side;
	...
	uint x = TileX(index) * TILE_SIZE + SignalPositions[side][pos].x;
	uint y = TileY(index) * TILE_SIZE + SignalPositions[side][pos].y;
 ...
 */
static void DrawTrackDetails(const TileInfo *ti, const RailTypeInfo *rti, bool draw_halftile)
{
	/* Base sprite for track fences.
	 * Note: Halftile slopes only have fences on the upper part. */
	uint num_sprites = 0;
	SpriteID base_image = GetCustomRailSprite(rti, ti->index, RTSG_FENCES, draw_halftile ? TCX_UPPER_HALFTILE : TCX_NORMAL, &num_sprites);
	if (base_image == 0) {
	...
	switch (GetRailFenceType(ti->tile)) {
		case RAIL_FENCE_NW:     DrawTrackFence_NW(ti, base_image, num_sprites); break;
		case RAIL_FENCE_SE:     DrawTrackFence_SE(ti, base_image, num_sprites); break;
		case RAIL_FENCE_SENW:   DrawTrackFence_NW(ti, base_image, num_sprites);
		                        DrawTrackFence_SE(ti, base_image, num_sprites); break;
		case RAIL_FENCE_NE:     DrawTrackFence_NE(ti, base_image, num_sprites); break;
		case RAIL_FENCE_SW:     DrawTrackFence_SW(ti, base_image, num_sprites); break;
		case RAIL_FENCE_NESW:   DrawTrackFence_NE(ti, base_image, num_sprites);
		                        DrawTrackFence_SW(ti, base_image, num_sprites); break;
		case RAIL_FENCE_VERT1:  DrawTrackFence(ti, base_image, num_sprites, RFO_FLAT_LEFT);  break;
		case RAIL_FENCE_VERT2:  DrawTrackFence(ti, base_image, num_sprites, RFO_FLAT_RIGHT); break;
		case RAIL_FENCE_HORIZ1: DrawTrackFence(ti, base_image, num_sprites, RFO_FLAT_UPPER); break;
		case RAIL_FENCE_HORIZ2: DrawTrackFence(ti, base_image, num_sprites, RFO_FLAT_LOWER); break;
		default: break;
	}
}
...
static void DrawTrackBitsOverlay(TileInfo *ti, TrackBits track, const RailTypeInfo *rti, bool draw_halftile, Corner halftile_corner)
{
	...
	SpriteID ground = GetCustomRailSprite(rti, ti->index, no_combine ? RTSG_GROUND_COMPLETE : RTSG_GROUND);
	...
	if (!draw_halftile) {
		...
		/* Draw higher halftile-overlay */
		overlay = GetCustomRailSprite(rti, ti->index, RTSG_OVERLAY, TCX_UPPER_HALFTILE);
		ground = GetCustomRailSprite(rti, ti->index, RTSG_GROUND, TCX_UPPER_HALFTILE);
 ...
 * @param halftile_corner The corner where the upper half-tile is or CORNER_INVALID if no half-tile.
 */
static void DrawTrackBits(TileInfo *ti, TrackBits track, bool draw_halftile, Corner halftile_corner)
{
	const RailTypeInfo *rti = GetRailTypeInfo(GetRailType(ti->tile));

	if (rti->UsesOverlay()) {
		DrawTrackBitsOverlay(ti, track, rti, draw_halftile, halftile_corner);
		return;
	}
	...
	if (!draw_halftile) {
		...
		SpriteID image;
		Slope track_slope = SLOPE_FLAT;
		if (ti->tileh != SLOPE_FLAT) {
			/* track on non-flat ground */
			image = _track_sloped_sprites[ti->tileh - 1] + rti->base_sprites.track_y;
			if (track & TRACK_BIT_UPPER) track_slope = SLOPE_N;
			if (track & TRACK_BIT_LOWER) track_slope = SLOPE_S;
			if (track & TRACK_BIT_RIGHT) track_slope = SLOPE_E;
			if (track & TRACK_BIT_LEFT)  track_slope = SLOPE_W;
		} else {
			...
			switch (track) {
				...
				default:
					junction = true;
					if ((track & TRACK_BIT_3WAY_NE) == 0) {
						image = rti->base_sprites.ground;     break;
					}
					if ((track & TRACK_BIT_3WAY_SW) == 0) {
						image = rti->base_sprites.ground + 1; break;
					}
					if ((track & TRACK_BIT_3WAY_NW) == 0) {
						image = rti->base_sprites.ground + 2; break;
					}
					if ((track & TRACK_BIT_3WAY_SE) == 0) {
						image = rti->base_sprites.ground + 3; break;
					}
		...
		switch (track) {
			case TRACK_BIT_UPPER: sub = GetHalftileSubSprite(CORNER_N, true); break;
			case TRACK_BIT_LOWER: sub = GetHalftileSubSprite(CORNER_S, true); break;
			case TRACK_BIT_RIGHT: sub = GetHalftileSubSprite(CORNER_E, true); break;
			case TRACK_BIT_LEFT:  sub = GetHalftileSubSprite(CORNER_W, true); break;
			default: break;
		}

		PaletteID pal = PAL_NONE;
		if (IsTileType(ti->index, MP_CLEAR)) {
			/* Adjust drawn sprite for better appearance. */
			if ((IsClearGround(ti->index, CLEAR_DESERT) || IsClearGround(ti->index, CLEAR_SNOW)) && GetClearDensity(ti->index) >= (ti->tileh & track_slope ? 1u : 2u)) {
				image += rti->snow_offset;
			} else if (GetClearDensity(ti->index) == 0) {
				pal = PALETTE_TO_BARE_LAND;
			}
		}
		DrawTrackSprite(image, pal, ti, track_slope, sub);
		...
		if (IsTileType(ti->index, MP_CLEAR)) {
			/* Adjust drawn sprite for better appearance. */
			if ((IsClearGround(ti->index, CLEAR_DESERT) || IsClearGround(ti->index, CLEAR_SNOW)) && GetClearDensity(ti->index) >= 1u) {
				image += rti->snow_offset;
			} else if (GetClearDensity(ti->index) == 0) {
				pal = PALETTE_TO_BARE_LAND;
			}
		...
		DrawGroundSprite(image, pal, GetHalftileSubSprite(halftile_corner, true), 0, -(int)TILE_HEIGHT);
...
static void DrawSignals(TileIndex index, Tile tile, TrackBits rails, const RailTypeInfo *rti)
{
...
static void DrawTile_Track(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	const RailTypeInfo *rti = GetRailTypeInfo(GetRailType(ti->tile));
	...
	if (IsPlainRail(ti->tile)) {
		...
		if (rails != TRACK_BIT_NONE) {
			DrawTrackBits(ti, rails, draw_halftile, halftile_corner);

			if (HasBit(_display_opt, DO_FULL_DETAIL)) DrawTrackDetails(ti, rti, draw_halftile);
		...
		Axis axis = GetTrackBits(ti->tile) == TRACK_BIT_X ? AXIS_X : AXIS_Y;

		/* Draw base sprite. */
		PaletteID pal = PAL_NONE;
		SpriteID image = rti->UsesOverlay() ? SPR_ROAD_Y + axis : rti->base_sprites.crossing + axis;
		if (!rti->UsesOverlay() && IsCrossingBarred(ti->tile)) image += 2;

		Roadside roadside = GetRoadside(ti->index);
		if (DrawRoadAsSnowOrDesert(IsOnSnowOrDesert(ti->index), roadside)) {
			image += rti->UsesOverlay() ? 19 : 8;
 		} else {
			switch (roadside) {
				case ROADSIDE_BARREN: pal = PALETTE_TO_BARE_LAND; break;
				case ROADSIDE_GRASS:  break;
				default:              image += rti->UsesOverlay() ? -19 : 4; break; // Paved
			}
		}
		DrawGroundSprite(image, pal);

		/* Build TileInfo that refers to the road sub-tile. */
		TileInfo road_ti(*ti);
		road_ti.tile = Tile::GetByType(ti->index, MP_ROAD);

		/* Draw any road overlays. */
		RoadType road_rt = GetRoadTypeRoad(ti->index);
		RoadType tram_rt = GetRoadTypeTram(ti->index);
		const RoadTypeInfo *road_rti = road_rt == INVALID_ROADTYPE ? nullptr : GetRoadTypeInfo(road_rt);
		const RoadTypeInfo *tram_rti = tram_rt == INVALID_ROADTYPE ? nullptr : GetRoadTypeInfo(tram_rt);
		DrawRoadOverlays(&road_ti, pal, road_rti, tram_rti, axis, axis);

		/* Draw rail/PBS overlay */
		bool draw_pbs = _game_mode != GM_MENU && _settings_client.gui.show_track_reservation && GetRailReservationTrackBits(ti->tile) != TRACK_BIT_NONE;
		if (rti->UsesOverlay()) {
			pal = draw_pbs ? PALETTE_CRASH : PAL_NONE;
			SpriteID rail = GetCustomRailSprite(rti, ti->index, RTSG_CROSSING) + axis;
			DrawGroundSprite(rail, pal);

			const Axis road_axis = OtherAxis(axis);
			...
			for (DiagDirection dir : { dir1, dir2 }) {
				...
				if (t < Map::Size() && IsLevelCrossingTile(t) && (GetTrackBits(GetLevelCrossingTile(t)) == TRACK_BIT_X ? AXIS_Y : AXIS_X) == road_axis) {
			...
			pal = draw_pbs ? PALETTE_CRASH : PAL_NONE;
			SpriteID rail = axis == AXIS_X ? GetRailTypeInfo(GetRailType(ti->tile))->base_sprites.single_x : GetRailTypeInfo(GetRailType(ti->tile))->base_sprites.single_y;
			DrawGroundSprite(rail, pal);
		}

		/* Draw road, tram catenary */
		DrawRoadCatenary(&road_ti);
		...
		if (rti->UsesOverlay()) {
			...
			int depot_sprite = GetCustomRailSprite(rti, ti->index, RTSG_DEPOT);
			...
			if (image != 0) {
				...
				/* Adjust ground tile for desert, don't adjust for snow, because snow in depots looks weird. */
				if (IsTileType(ti->index, MP_CLEAR) && IsClearGround(ti->index, CLEAR_DESERT) && GetClearDensity(ti->index) >= 2) image += rti->snow_offset;
			}
			relocation = rti->GetRailtypeSpriteOffset();
		}

		if (image != 0) DrawGroundSprite(image, GroundSpritePaletteTransform(image, pal, _drawtile_track_palette));
...
void DrawTrainDepotSprite(int x, int y, int dir, RailType railtype)
{
	...
	if (image == 0) image = SPR_FLAT_GRASS_TILE;
...
static Foundation GetFoundation_Track(TileIndex, Tile tile, Slope tileh)
{
...
static TrackBits GetOwnTrackBits(TileIndex index, Owner o)
{
	...
	for (Tile rail : RailTileIterator::Iterate(index)) {
		if (IsTileOwner(rail, o)) {
			...
			if (IsRailDepot(rail)) bits |= TRACK_BIT_CROSS;
			if (IsNormalRail(rail)) bits |= GetTrackBits(rail);
		}
	}
	return bits;
}
...
static bool TileLoop_Track(TileIndex index, Tile &tile)
{
	...
	if (IsPlainRail(tile) && (!IsTileType(index, MP_CLEAR) || GetClearDensity(index) == 3 || GetClearGround(index) == CLEAR_DESERT || GetClearGround(index) == CLEAR_SNOW)) { // wait until bottom is green
		...
		for (DiagDirection d = DIAGDIR_BEGIN; d < DIAGDIR_END; d++) {
			...
			Tile rail2 = GetRailTileFromDiagDir(tile2, d);
			if (!rail2) rail2 = Tile::GetByType(tile2, MP_RAILWAY);

			/* Show fences if it's a house, industry, object, road, tunnelbridge or not owned by us. */
			if (!IsValidTile(tile2) || IsTileType(tile2, MP_HOUSE) || IsTileType(tile2, MP_INDUSTRY) ||
					IsTileType(tile2, MP_ROAD) || (IsTileType(tile2, MP_OBJECT) && !IsObjectType(tile2, OBJECT_OWNED_LAND)) || IsTileType(tile2, MP_TUNNELBRIDGE) || !IsTileOwner(rail2 ? rail2 : tile2, owner)) {
				fences |= 1 << d;
			}
		}
		...
		switch (fences) {
			case 0: break;
			case (1 << DIAGDIR_NE): new_fences = RAIL_FENCE_NE; break;
			case (1 << DIAGDIR_SE): new_fences = RAIL_FENCE_SE; break;
			case (1 << DIAGDIR_SW): new_fences = RAIL_FENCE_SW; break;
			case (1 << DIAGDIR_NW): new_fences = RAIL_FENCE_NW; break;
			case (1 << DIAGDIR_NE) | (1 << DIAGDIR_SW): new_fences = RAIL_FENCE_NESW; break;
			case (1 << DIAGDIR_SE) | (1 << DIAGDIR_NW): new_fences = RAIL_FENCE_SENW; break;
			case (1 << DIAGDIR_NE) | (1 << DIAGDIR_SE): new_fences = RAIL_FENCE_VERT1; break;
			case (1 << DIAGDIR_NE) | (1 << DIAGDIR_NW): new_fences = RAIL_FENCE_HORIZ2; break;
			case (1 << DIAGDIR_SE) | (1 << DIAGDIR_SW): new_fences = RAIL_FENCE_HORIZ1; break;
			case (1 << DIAGDIR_SW) | (1 << DIAGDIR_NW): new_fences = RAIL_FENCE_VERT2; break;
			default: NOT_REACHED();
		}
	}

	if (old_fences != new_fences) {
		SetRailFenceType(tile, new_fences);
		MarkTileDirtyByTile(index);
	}
	return false;
}
...
static TrackStatus GetTileTrackStatus_Track(TileIndex index, Tile tile, TransportType mode, uint, DiagDirection side)
{
	if (mode == TRANSPORT_ROAD && IsLevelCrossing(tile)) {
		/* Only return the "red light" part, road bits is done by the road tile handler. */
		Axis axis = GetTrackBits(tile) == TRACK_BIT_X ? AXIS_Y : AXIS_X;
		if (side != INVALID_DIAGDIR && axis != DiagDirToAxis(side)) return 0;
		if (!IsCrossingBarred(tile)) return 0;

		TrackdirBits red_signals = TrackBitsToTrackdirBits(AxisToTrackBits(axis));

		if (!TrainOnCrossing(index)) {
			auto mask_red_signal_bits_if_crossing_barred = [&](TileIndex t, TrackdirBits mask) {
				if (IsLevelCrossingTile(t) && IsCrossingBarred(GetLevelCrossingTile(t))) red_signals &= mask;
			};
		...
		return CombineTrackStatus(TRACKDIR_BIT_NONE, red_signals);
	}
...
static bool ClickTile_Track(TileIndex index, Tile tile)
{
	if (!IsRailDepot(tile)) return false;

	ShowDepotWindow(index, VEH_TRAIN);
	return true;
}
...
static bool ChangeTileOwner_Track(TileIndex index, Tile &tile, Owner old_owner, Owner new_owner)
{
	...
	if (new_owner != INVALID_OWNER) {
		...
		if (IsLevelCrossing(tile)) num_pieces *= LEVELCROSSING_TRACKBIT_FACTOR;
		...
		return std::get<1>(ClearTile_Track(index, tile, {DoCommandFlag::Execute, DoCommandFlag::Bankrupt}));
	}
}
 ...
 */
static VehicleEnterTileStates VehicleEnter_Track(Vehicle *u, TileIndex index, Tile tile, int x, int y)
{
...
static CommandCost TestAutoslopeOnRailTile(TileIndex tile, DoCommandFlags flags, int z_old, Slope tileh_old, int z_new, Slope tileh_new, TrackBits rail_bits)
{
	...
	if (tileh_old != tileh_new) {
		...
		if (IsTileType(tile, MP_WATER) && IsSlopeWithOneCornerRaised(tileh_old)) cost.AddCost(_price[PR_CLEAR_WATER]);
		if (flags.Test(DoCommandFlag::Execute)) MakeClearGrass(tile);
	}
	return  cost;
}

static CommandCost TerraformTile_Track(TileIndex index, Tile tile, DoCommandFlags flags, int z_new, Slope tileh_new)
{
	auto [tileh_old, z_old] = GetTileSlopeZ(index);
	if (IsPlainRail(tile)) {
		TrackBits rail_bits = GetTrackBits(tile);
		/* Is there flat water on the lower halftile that must be cleared expensively? */
		bool was_water = (IsTileType(index, MP_WATER) && IsSlopeWithOneCornerRaised(tileh_old));
		...
		if (IsLevelCrossing(tile)) {
			if (!IsSteepSlope(tileh_new) && (GetTileMaxZ(index) == z_new + GetSlopeMaxZ(tileh_new)) && HasBit(VALID_LEVEL_CROSSING_SLOPES, tileh_new)) return CommandCost(EXPENSES_CONSTRUCTION, _price[PR_BUILD_FOUNDATION]);
		} else if (AutoslopeCheckForEntranceEdge(index, z_new, tileh_new, GetRailDepotDirection(tile))) {
			return CommandCost(EXPENSES_CONSTRUCTION, _price[PR_BUILD_FOUNDATION]);
		}
	}

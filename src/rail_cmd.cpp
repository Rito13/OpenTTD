...
static constexpr SlopeIndexArray<uint8_t, 15> _track_sloped_sprites = {
...
static constexpr SlopeIndexArray<TrackBits, 15> _valid_tracks_without_foundation = {
	TRACK_BIT_ALL,
	Track::Right,
	Track::Upper,
	Track::X,
...
static constexpr SlopeIndexArray<TrackBits, 15> _valid_tracks_on_leveled_foundation = {{{
	{},
	Track::Left,
	Track::Lower,
...
static void DrawTrackBits(TileInfo *ti, TrackBits track, bool draw_halftile, Corner halftile_corner)
{
	...
	const SubSprite *sub = track.Any(TRACK_BIT_CROSS) ? nullptr : GetHalftileSubSprite(TrackBitsToCorner(track), true);
...
static CommandCost TestAutoslopeOnRailTile(TileIndex tile, DoCommandFlags flags, int z_old, Slope tileh_old, int z_new, Slope tileh_new, TrackBits rail_bits)
{
	...
	if (tileh_old != tileh_new) {
		...
		if (IsTileType(tile, TileType::Water) && IsSlopeWithOneCornerRaised(tileh_old)) cost.AddCost(_price[Price::ClearWater]);
...
static CommandCost TerraformTile_Rail(TileIndex index, const Tile &tile, DoCommandFlags flags, int z_new, Slope tileh_new)
{
	...
	if (IsPlainRail(tile)) {
		...
		bool was_water = (IsTileType(index, TileType::Water) && IsSlopeWithOneCornerRaised(tileh_old));

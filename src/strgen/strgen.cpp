...
struct ReplacableFileWriter : FileWriter {
	 ...
	 * @param path The path to the file to open.
	 */
	ReplacableFileWriter(const std::filesystem::path &path) : FileWriter("tmp.xxx", std::ofstream::out),
		real_path(path)
	{
...
struct HeaderFileWriterBase : HeaderWriter, ReplacableFileWriter {
	 ...
	 * @param path The path to the file to open.
	 */
	HeaderFileWriterBase(const std::filesystem::path &path) : ReplacableFileWriter(path),
	 ...
	 * @return The plural form with the most amount of cases.
	 */
	size_t FindMostCasesPlural()
	{
		size_t max_plural_forms = 0;
		for (const auto &pf : _plural_forms) {
			max_plural_forms = std::max(max_plural_forms, pf.plural_count);
		}
		return max_plural_forms;
	}
};

#ifdef MODULES_ARE_SUPPORTED

struct HeaderFileWriter : ReplacableFileWriter {
	/**
	 * Open a file to write to.
	 * @param path The path to the file to open.
	 */
	HeaderFileWriter(const std::filesystem::path &path) : ReplacableFileWriter(path)
	{
	 ...
	 */
	void WriteHeader([[maybe_unused]] const StringData &data)
	{
		this->output_stream << "import Strings;\n";
		this->output_stream << "using namespace Strings;\n";
		this->output_stream << "static_assert(STR_NULL == STR_NULL);\n";
	}

	void Finalise([[maybe_unused]] const StringData &data)
	{
...
struct SourceFileWriter : HeaderFileWriterBase {
	/**
	 * Open a file to write to.
	 * @param path The path to the file to open.
	 */
	SourceFileWriter(const std::filesystem::path &path) : HeaderFileWriterBase(path)
	{
		this->output_stream << "module;\n";
		/* Including stdafx.h couses internall errors in gcc older than 15.0. */
		this->output_stream << "#include <cstdint>\n";
		this->output_stream << "export module Strings;\n";
		this->output_stream << "\n";
		/* Because strings_type.h depends on stdafx.h, it can't be included. */
		this->output_stream << "export using StringID = uint32_t;\n";
		this->output_stream << "export namespace Strings {\n";
	}

	void WriteStringID(const std::string &name, size_t stringid) override
	{
		if (prev + 1 != stringid) this->output_stream << "\n";
		/* Has to explicitly define the extern annotation or the gcc older that 15.0 won't find the constants. */
		fmt::print(this->output_stream, "\textern const StringID {} = 0x{:X};\n", name, stringid);
		prev = stringid;
		total_strings++;
	}
	...
	{
		size_t max_plural_forms = this->FindMostCasesPlural();

		/* Has to explicitly define the extern annotation or the gcc older that 15.0 won't find the constants. */
		fmt::print(this->output_stream,
			"\n"
			"\textern const unsigned int LANGUAGE_PACK_VERSION     = 0x{:X};\n"
			"\textern const unsigned int LANGUAGE_MAX_PLURAL       = {};\n"
			"\textern const unsigned int LANGUAGE_MAX_PLURAL_FORMS = {};\n"
			"\textern const unsigned int LANGUAGE_TOTAL_STRINGS    = {};\n"
			"{}\n",
...
struct HeaderFileWriter : HeaderFileWriterBase {
	/**
	 * Open a file to write to.
	 * @param path The path to the file to open.
	 */
	HeaderFileWriter(const std::filesystem::path &path) : HeaderFileWriterBase(path)
	{
		this->output_stream << "#ifndef TABLE_STRINGS_H\n";
		this->output_stream << "#define TABLE_STRINGS_H\n";
	}
...
struct SourceFileWriter : ReplacableFileWriter {
	/**
	 * Open a file to write to.
	 * @param path The path to the file to open.
	 */
	SourceFileWriter(const std::filesystem::path &path) : ReplacableFileWriter(path) {}
...
int CDECL main(int argc, char *argv[])
{
	...
	try {
		...
		if (mgo.arguments.empty()) {
			...
			/* write Strings.cpp
			 * Source file has to have the same name as module or the clang won't compile. */
			output_path = dest_dir;
			output_path /= "Strings.cpp";

			SourceFileWriter source(output_path);
			source.WriteHeader(data);

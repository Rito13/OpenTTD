...
{
	VehicleEnterTileStatus vets = VETSB_CONTINUE;

	/* Loop through all associated tiles until we get a non-continue result. */
	for (Tile t = tile; t.IsValid() && vets == VETSB_CONTINUE; ++t) {
...
void Vehicle::ShowVisualEffect() const
{
	...
	do {
		...
		if (effect_model == VESM_NONE ||
				...
				IsTunnelTile(v->tile) ||
				(v->type == VEH_TRAIN &&
				!HasPowerOnRail(Train::From(v)->railtype, GetTileRailType(v->tile, TrackdirToTrack(v->GetVehicleTrackdir()))))) {
			continue;
		}

...
static void PlaceExtraDepotRail(TileIndex index, DiagDirection dir, Track track)
{
	...
	Command<Commands::BuildRail>::Post(index, _cur_railtype, track, _settings_client.gui.auto_remove_signals);
}
...
void CcRailDepot(Commands, const CommandCost &result, TileIndex tile, RailType, DiagDirection dir)
{
	...
	if (Tile::HasType(tile, TileType::Railway)) {
		...
		Tile double_depot_tile = Tile::GetByType(tile + TileOffsByDiagDir(dir), TileType::Railway);
		bool is_double_depot = IsRailDepotTile(double_depot_tile);

...
#	define LANDINFOD_LEVEL 1
		...
		static constexpr EnumClassIndexContainer<std::array<std::string_view, to_underlying(TileType::End)>, TileType> ttype_to_str {
			"Clear"sv,
			"Railway"sv,
			"Road"sv,
			"House"sv,
		...
		do {
			Debug(misc, LANDINFOD_LEVEL, "type   = 0x{:x} ({})", t.type(), t.GetTileType() < TileType::End ? ttype_to_str[t.GetTileType()] : "Invalid"sv);
			Debug(misc, LANDINFOD_LEVEL, "height = 0x{:x}", t.height());
			Debug(misc, LANDINFOD_LEVEL, "m1     = 0x{:x}", t.m1());
			Debug(misc, LANDINFOD_LEVEL, "m2     = 0x{:x}", t.m2());
			Debug(misc, LANDINFOD_LEVEL, "m3     = 0x{:x}", t.m3());
			Debug(misc, LANDINFOD_LEVEL, "m4     = 0x{:x}", t.m4());
			Debug(misc, LANDINFOD_LEVEL, "m5     = 0x{:x}", t.m5());
			Debug(misc, LANDINFOD_LEVEL, "m6     = 0x{:x}", t.m6());
			Debug(misc, LANDINFOD_LEVEL, "m7     = 0x{:x}", t.m7());
			Debug(misc, LANDINFOD_LEVEL, "m8     = 0x{:x}", t.m8());
		} while (++t);
...
#undef LANDINFOD_LEVEL
	...
	{
		Town *town = ClosestTownFromTile(this->tile, _settings_game.economy.dist_local_authority);
		...
		uint64_t prev_param = 0;
		for (const TileDesc &td : tds) {
			if (td.str == prev_str && td.dparam == prev_param) continue;

			/* Add a comma between each item. */
			if (tt_str.tellp() > 0) tt_str << ", ";

			prev_str = td.str;
			prev_param = td.dparam;
			tt_str << GetString(prev_str, prev_param);
		}
		if (tt_str.tellp() > 0) this->landinfo_data.push_back(tt_str.str());

		/* Owners */
		bool has_owner = false;
		for (Owner prev_owner = INVALID_OWNER; const TileDesc &td : tds) {
			/* Up to four owners per tile. */
			for (uint i = 0; i < 4; i++) {
		...
		if (!has_owner) {
			...
			this->landinfo_data.push_back(GetString(STR_LAND_AREA_INFORMATION_OWNER, STR_LAND_AREA_INFORMATION_OWNER_N_A, std::monostate{}));
		}
		...
		if (town == nullptr) {
			...
			this->landinfo_data.push_back(GetString(STR_LAND_AREA_INFORMATION_LOCAL_AUTHORITY, STR_TOWN_NAME, town->index));
		}

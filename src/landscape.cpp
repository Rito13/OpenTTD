...
uint GetPartialPixelZ(int x, int y, Slope corners)
{
	...
	switch (RemoveHalftileSlope(corners).base()) {
		...
		/* One corner is up.*/
		case Slope{BaseSlope::N}.base(): return x + y <= static_cast<int>(TILE_SIZE) ? (TILE_SIZE - x - y) >> 1 : 0;
		case Slope{BaseSlope::E}.base(): return y >= x ? (1 + y - x) >> 1 : 0;
		case Slope{BaseSlope::S}.base(): return x + y >= static_cast<int>(TILE_SIZE) ? (1 + x + y - TILE_SIZE) >> 1 : 0;
		case Slope{BaseSlope::W}.base(): return x >= y ? (x - y) >> 1 : 0;
...
void GetSlopePixelZOnEdge(Slope tileh, DiagDirection edge, int &z1, int &z2)
{
	static const Slope corners[4][4] = {
		/*    corner     |          steep slope
		 *  z1      z2   |       z1             z2        */
		{BaseSlope::E, BaseSlope::N, SLOPE_STEEP_E, SLOPE_STEEP_N}, // DIAGDIR_NE, z1 = E, z2 = N
		{BaseSlope::S, BaseSlope::E, SLOPE_STEEP_S, SLOPE_STEEP_E}, // DIAGDIR_SE, z1 = S, z2 = E
		{BaseSlope::S, BaseSlope::W, SLOPE_STEEP_S, SLOPE_STEEP_W}, // DIAGDIR_SW, z1 = S, z2 = W
		{BaseSlope::W, BaseSlope::N, SLOPE_STEEP_W, SLOPE_STEEP_N}, // DIAGDIR_NW, z1 = W, z2 = N
	};
...
void DrawFoundation(TileInfo *ti, Foundation f)
{
	...
	if (IsSteepSlope(ti->tileh)) {
		if (!IsNonContinuousFoundation(f)) {
			...
			AddSortableSpriteToDraw(leveled_base + Slope(ti->tileh).Reset(BaseSlope::Steep).base(), PAL_NONE, *ti, bounds);
		}

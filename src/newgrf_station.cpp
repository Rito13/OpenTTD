...
/* virtual */ uint32_t StationScopeResolver::GetVariable(uint8_t variable, [[maybe_unused]] uint32_t parameter, bool &available) const
{
		...
		switch (variable) {
			...
			case 0x67: // Land info of nearby tile
				if (this->axis != INVALID_AXIS && this->tile != INVALID_TILE) {
					...
					bool swap = (this->axis == AXIS_Y && tileh.Test(BaseSlope::W) != tileh.Test(BaseSlope::E));
	...
	switch (variable) {
		...
		case 0x67: { // Land info of nearby tile
			...
			Slope tileh = GetTileSlope(tile);
			bool swap = (axis == AXIS_Y && tileh.Test(BaseSlope::W) != tileh.Test(BaseSlope::E));
...
CommandCost PerformStationTileSlopeCheck(TileIndex north_tile, TileIndex cur_tile, const StationSpec *statspec, Axis axis, uint8_t plat_len, uint8_t numtracks)
{
	...
	StationResolverObject object(statspec, nullptr, cur_tile, CBID_STATION_LAND_SLOPE_CHECK,
			(slope.base() << 4) | Slope(slope).Flip(axis == AXIS_Y && slope.Test(BaseSlope::W) != slope.Test(BaseSlope::E) ? SLOPE_EW : SLOPE_FLAT).base(),

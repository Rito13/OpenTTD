...
{
	assert(IsTileType(tile, TileType::Clear) || IsTileType(tile, TileType::Water));
	...
	if (Tile::HasType(tile, TileType::Trees)) tt = TileType::Trees;

	uint32_t param1_v7 = to_underlying(tt) << 28 | Clamp(TileHeight(tile), 0, 15) << 24 | GB(r, 16, 8) << 16 | GetTerrainType(tile);
	uint32_t param1_v8 = to_underlying(tt) << 24 | GetTileZ(tile) << 16 | GB(r, 16, 8) << 8 | (HasTileWaterClass(tile) ? to_underlying(GetWaterClass(tile)) : 0) << 3 | GetTerrainType(tile);

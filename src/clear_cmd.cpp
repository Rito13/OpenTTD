...
void DrawClearLandTile(const TileInfo *ti, uint8_t density, bool draw_halftile, Corner halftile_corner)
{
	...
	DrawGroundSprite(_clear_land_sprites_grass[density] + offset, PAL_NONE, subsprite);
}
...
/**
 * Draw a ClearGround::Desert tile.
 * @copydetails DrawTileProc
 */
...
static inline void DrawSnowTile(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (GetClearGround(ti->tile) == ClearGround::Rocks) {
		DrawGroundSprite(_clear_land_sprites_snowy_rocks[density] + offset, PAL_NONE, subsprite);
	}
}

/**
 * Draw a ClearGround::Fields tile.
 * @copydetails DrawTileProc
 */
...
static void DrawTile_Clear(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	switch (GetClearGround(ti->tile)) {
		...
		case ClearGround::Rough:
			DrawRoughLandTile(ti, draw_halftile, halftile_corner);
			break;

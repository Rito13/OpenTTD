...
void SetWaterClassDependingOnSurroundings(TileIndex t, bool include_invalid_water_class)
{
	...
	for (DiagDirection dir : EnumRange(DiagDirection::End)) {
		...
		switch (GetTileType(neighbour)) {
			...
			case TileType::Trees:
				/* This function is only called for old savegames which still have trees on shore as TileType::Trees tiles. */
				has_water |= (GB(neighbour.m2(), 4, 2) == 3 /* TreeGround::Shore */);
				break;
...
void DecomposeTile(TileIndex index)
{
	switch (GetTileType(index)) {
		...
		case TileType::Trees: {
			...
			new_tile.GetTileBase() = old_tile.GetTileBase();
			new_tile.GetTileExtended() = old_tile.GetTileExtended();
			ClrBit(new_tile.m8(), 14); // Clear out any garbage in the associated tile flag.

			/* Clear now unused storage of the new tile. */
			uint8_t ground_type = GB(new_tile.m2(), 6, 3);
			...
			switch (ground_type) {
				case 0: // Clear land
				case 1: // Rough land
					MakeClear(old_tile, static_cast<ClearGround>(ground_type), density);
					break;
				case 2: // Snow or desert
					MakeClear(old_tile, _settings_game.game_creation.landscape == LandscapeType::Tropic ? ClearGround::Desert : ClearGround::Grass, density);
					if (_settings_game.game_creation.landscape == LandscapeType::Arctic) MakeSnow(old_tile, density);
					break;
				case 3: // Shore tile
					MakeShore(old_tile);
					break;
				case 4: // Rough snow
					MakeClear(old_tile, ClearGround::Rough, 3);
...
static void FixOwnerOfRailTrack(TileIndex ti, const Tile &t)
{
	...
	for (DiagDirection dd : EnumRange(DiagDirection::End)) {
		...
		if (GetTileTrackStatus(ti, TransportType::Rail, RoadTramType::Invalid, dd).trackdirs.Any() &&
...
bool AfterLoadGame()
{
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::VehicleCurrencyStationChanges)) {
		...
		for (auto ti : Map::IterateIndex()) {
			...
			int dx = TileX(ti) - TileX(st->train_station.tile);
			int dy = TileY(ti) - TileY(st->train_station.tile);
			assert(dx >= 0 && dy >= 0);
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::MultipleRoadStops, 1)) {
		for (auto ti : Map::IterateIndex()) {
			...
			switch (GetTileType(t)) {
				case TileType::House:
					t.m4() = t.m2();
					SetTownIndex(t, CalcClosestTownFromTile(ti)->index);
					break;
			...
			switch (GetTileType(t)) {
				case TileType::Road:
					...
					if (!IsRoadDepot(t) && !HasTownOwnedRoad(t)) {
						const Town *town = CalcClosestTownFromTile(ti);
				...
				if (HasBit(t.m5(), 6)) { // middle part
					...
					if (HasBit(t.m5(), 5)) { // transport route under bridge?
						if (static_cast<TransportType>(GB(t.m5(), 3, 2)) == TransportType::Rail) {
							...
							TownID town = IsTileOwner(t, OWNER_TOWN) ? ClosestTownFromTile(ti, UINT_MAX)->index : TownID::Begin();
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::StatueOwner)) {
		for (auto ti : Map::IterateIndex()) {
			...
			if (IsTileType(t, TileType::Object) && t.m5() == OBJECT_STATUE) {
				t.m2() = CalcClosestTownFromTile(ti)->index.base();
			}
		}
	}
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::FixTreeGround)) {
		for (auto t : Map::Iterate()) {
			if (GetTileType(t) == TileType::Trees) {
				uint ground_type = GB(t.m2(), 4, 2);
				if (ground_type != 2 /* TreeGround::SnowOrDesert */) SB(t.m2(), 6, 2, 3);
			}
		}
	}
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::SimplifyPathfinderSettings)) {
		for (const auto ti : Map::IterateIndex()) {
			...
			if ((IsTileType(t, TileType::Water) || IsBuoyTile(t)) &&
					(TileX(ti) == 0 || TileY(ti) == 0 || TileX(ti) == Map::MaxX() - 1 || TileY(ti) == Map::MaxY() - 1)) {
			...
			if (IsBuoyTile(t) || IsDriveThroughStopTile(t) || IsTileType(t, TileType::Water)) {
				...
				if (o < MAX_COMPANIES && !Company::IsValidID(o)) {
					...
					ChangeTileOwner(ti, o, INVALID_OWNER);
				}
				if (IsBuoyTile(t)) {
				...
				if (IsLevelCrossing(t)) {
					if (!Company::IsValidID(GetTileOwner(t))) FixOwnerOfRailTrack(ti, t);
				}
		...
		for (auto ti : Map::IterateIndex()) {
			...
			if (Town::GetNumItems() == 0) {
				...
				if (offset == 0) {
					...
					o->town = type == OBJECT_STATUE ? Town::Get(t.m2()) : CalcClosestTownFromTile(ti, UINT_MAX);
					t.m2() = o->index.base();
				} else {
	...
	if (IsSavegameVersionBefore(SaveLoadVersion::VehicleCentreAndZPos) && !IsSavegameVersionBefore(SaveLoadVersion::LinkFarmFieldToIndustry)) {
		...
		for (auto ti : Map::IterateIndex()) {
			...
			if (fence != 0 && IsTileType(TileAddXY(ti, 1, 0), TileType::Clear) && IsClearGround(TileAddXY(ti, 1, 0), ClearGround::Fields)) {
				SetFence(TileAddXY(ti, 1, 0), DiagDirection::NE, fence);
			}
			fence = GB(t.m4(), 2, 3);
			if (fence != 0 && IsTileType(TileAddXY(ti, 0, 1), TileType::Clear) && IsClearGround(TileAddXY(ti, 0, 1), ClearGround::Fields)) {
				SetFence(TileAddXY(ti, 0, 1), DiagDirection::NW, fence);
			}
			SB(t.m4(), 2, 3, 0);
			SB(t.m4(), 5, 3, 0);
		}

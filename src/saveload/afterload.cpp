...
bool AfterLoadGame()
{
	...
	if (IsSavegameVersionBefore(SLV_BETTER_TREES_STORAGE)) {
		for (auto t : Map::Iterate()) {
			...
			t.m3() = t.m5(); // Move tree growth and quantity in order to make space for ground and density in m5.
			...
			SB(t.m3(), 3, 3, type & 0b111); // Mask is necessary to prevent overriding the quantity.
			if (type >> 3) SetBit(t.m5(), 7);
		}
	}

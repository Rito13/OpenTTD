...
enum class ToolbarMode : uint8_t {
	...
	Upper, ///< Toolber is in splitted mode and the first part is selected.
	Lower, ///< Toolber is in splitted mode and the second part is selected.
};
...
enum class OptionMenuEntries : uint8_t {
	...
	ShowShipStationNames, ///< Toggle visibility of dock names.
	ShowPlaneStationNames, ///< Toggle visibility of airport names.
...
static CallBackFunction ToolbarOptionsClick(Window *w)
{
	...
	list.push_back(MakeDropDownListCheckedItem(_facility_display_opt.Test(StationFacility::Dock), STR_SETTINGS_MENU_STATION_NAMES_SHIP, OptionMenuEntries::ShowShipStationNames, false, false, 1));
	list.push_back(MakeDropDownListCheckedItem(_facility_display_opt.Test(StationFacility::Airport), STR_SETTINGS_MENU_STATION_NAMES_PLANE, OptionMenuEntries::ShowPlaneStationNames, false, false, 1));
...
static CallBackFunction MenuClickSettings(int index)
{
	switch (OptionMenuEntries(index)) {
		...
		case OptionMenuEntries::ShowShipStationNames: _facility_display_opt.Flip(StationFacility::Dock); break;
		case OptionMenuEntries::ShowPlaneStationNames: _facility_display_opt.Flip(StationFacility::Airport); break;
...
static CallBackFunction MenuClickSaveLoad(int index = 0)
{
	if (_game_mode == GM_EDITOR) {
		switch (SaveLoadEditorMenuEntries(index)) {
			...
			case SaveLoadEditorMenuEntries::ExitToIntro: AskExitToGameMenu(); break;
		...
		switch (SaveLoadNormalMenuEntries(index)) {
			...
			case SaveLoadNormalMenuEntries::ExitToIntro: AskExitToGameMenu(); break;
 ...
 * @todo GenerateLand behaves in the same way as Terraform, maybe GenerateLand should open world generation window instead?
 */

...
static CallBackFunction ToolbarBuildRoadClick(Window *w)
{
	if (_replace_dropdown_list) {
		...
		ShowDropDownList<RoadTypeDropdownWindowBase>(w, {}, _last_built_roadtype, WID_TN_ROADS, 140, _settings_client.gui.toolbar_dropdown_autoselect, true);
	}
	return CBF_NONE;
}
...
static CallBackFunction ToolbarBuildTramClick(Window *w)
{
	if (_replace_dropdown_list) {
		...
		ShowDropDownList<TramTypeDropdownWindowBase>(w, {}, _last_built_tramtype, WID_TN_TRAMS, 140, _settings_client.gui.toolbar_dropdown_autoselect, true);
	}
	return CBF_NONE;
}
...
struct MainToolbarWindow : Window {
	...
	void OnDropdownSelect(WidgetID widget, int index, [[maybe_unused]] int click_result) override
	{
		...
		if (cbf == CBF_REOPEN_DROPDOWN) {
			_replace_dropdown_list = true;
			this->OnClick({}, widget, 1);
			_replace_dropdown_list = false;
		} else if (cbf != CBF_NONE) {

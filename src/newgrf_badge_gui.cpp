...
using DropDownListBadgeItem = DropDownCustomSelectedBGColour<DropDownBadges<DropDownString<DropDownSpacer<DropDownListStringItem, true>, FS_SMALL, true>>>;
using DropDownListBadgeIconItem = DropDownCustomSelectedBGColour<DropDownBadges<DropDownString<DropDownSpacer<DropDownListIconItem, true>, FS_SMALL, true>>>;

std::unique_ptr<DropDownListItem> MakeDropDownListBadgeItem(const std::shared_ptr<GUIBadgeClasses> &gui_classes, std::span<const BadgeID> badges, GrfSpecFeature feature, std::optional<TimerGameCalendar::Date> introduction_date, std::string &&str, int value, bool masked, bool shaded, std::optional<Colours> ctrl_bg_colour, std::optional<Colours> shift_bg_colour)
{
	...
	return std::make_unique<DropDownListBadgeItem>(
			false, shift_colour, ctrl_colour,
			gui_classes, badges, feature, introduction_date, "",

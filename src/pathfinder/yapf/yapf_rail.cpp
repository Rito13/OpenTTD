...
private:
	...
	bool ReserveSingleTrack(TileIndex tile, Trackdir td)
	{
		...
		if (IsRailStationTile(tile)) {
			...
			if (Tile rail = GetRailTileFromTrack(tile, TrackdirToTrack(rev_td)); HasPbsSignalOnTrackdir(rail, rev_td) && GetSignalStateByTrackdir(rail, rev_td) == SignalState::Green) {
		...
		if (_debug_desync_level >= 2) {
			...
			if (result1.tile != result2.tile || (result1.reverse != result2.reverse)) {
				Debug(desync, 2, "warning: FindNearestDepotTwoWay cache mismatch: {} vs {}",
						result1.index != INVALID_TILE ? "T" : "F",
						result2.index != INVALID_TILE ? "T" : "F");
				DumpState(pf1, pf2);
			}
		}

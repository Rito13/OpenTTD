...
TrackBits GetReservedTrackbits(TileIndex t)
{
	if (auto rail_iter = RailTileIterator::Iterate(t); !rail_iter.empty()) {
		...
		for (Tile rail : rail_iter) {
			bits |= GetReservedRailTracks(rail);
		}
		return bits;
	}
...
static PBSTileInfo FollowReservation(Owner o, RailTypes rts, TileIndex tile, Trackdir trackdir, bool ignore_oneway = false)
{
	...
	while (ft.Follow(tile, trackdir)) {
		TrackdirBits reserved = ft.new_td_bits & TrackBitsToTrackdirBits(IsTileType(ft.new_sub_tile, MP_RAILWAY) ? GetReservedRailTracks(ft.new_sub_tile) : GetReservedTrackbits(ft.new_tile));
...
Train *GetTrainForReservation(TileIndex tile, Track track)
{
	...
	for (int i = 0; i < 2; ++i, trackdir = ReverseTrackdir(trackdir)) {
		...
		if (HasOnewaySignalBlockingTrackdir(rail_tile, ReverseTrackdir(trackdir)) && !HasPbsSignalOnTrackdir(rail_tile, trackdir)) continue;

		FindTrainOnTrackInfo ftoti;
		ftoti.res = FollowReservation(GetTileOwner(rail_tile.IsValid() ? rail_tile : tile), rts, tile, trackdir, true);
...
bool IsSafeWaitingPosition(const Train *v, TileIndex tile, Trackdir trackdir, bool include_line_end, bool forbid_90deg)
{
	...
	if (Rail90DegTurnDisallowed(GetTileRailType(ft.old_tile, TrackdirToTrack(ft.old_td)), GetRailType(ft.new_sub_tile), forbid_90deg)) ft.new_td_bits &= ~TrackdirCrossesTrackdirs(trackdir);
	...
	if (ft.new_td_bits != TRACKDIR_BIT_NONE && KillFirstBit(ft.new_td_bits) == TRACKDIR_BIT_NONE) {
		...
		if (HasPbsSignalOnTrackdir(ft.new_sub_tile, td)) return true;
		/* One-way PBS signal against us? Safe if end-of-line is allowed. */
		if (IsPlainRailTile(ft.new_sub_tile) && HasSignalOnTrackdir(ft.new_sub_tile, ReverseTrackdir(td)) && GetSignalType(ft.new_sub_tile, TrackdirToTrack(td)) == SIGTYPE_PBS_ONEWAY) {
...
bool IsWaitingPositionFree(const Train *v, TileIndex tile, Trackdir trackdir, bool forbid_90deg)
{
	...
	reserved = IsTileType(ft.new_sub_tile, MP_RAILWAY) ? GetReservedRailTracks(ft.new_sub_tile) : GetReservedTrackbits(ft.new_tile);
	return (reserved & TrackdirBitsToTrackBits(ft.new_td_bits)) == TRACK_BIT_NONE;
}

...
static SigFlags ExploreSegment(Owner owner)
{
	...
	while (_tbdset.Get(&index, &enterdir)) { // tile and enterdir are initialized here, unless I'm mistaken.
			...
			case MP_RAILWAY: {
				...
				if (IsRailDepot(tile)) {
					if (enterdir == INVALID_DIAGDIR) { // from 'inside' - train just entered or left the depot
						if (!(flags & SF_TRAIN) && HasVehicleOnPos(index, nullptr, &TrainOnTileEnum)) flags |= SF_TRAIN;
				...
				if (tracks == TRACK_BIT_HORZ || tracks == TRACK_BIT_VERT) { // there is exactly one incidating track, no need to check
					...
					if (!(flags & SF_TRAIN) && EnsureNoTrainOnTrackBits(index, tracks).Failed()) flags |= SF_TRAIN;
				} else {
				...
				for (DiagDirection dir = DIAGDIR_BEGIN; dir < DIAGDIR_END; dir++) { // test all possible exit directions
					if (dir != enterdir && (tracks & _enterdir_to_trackbits[dir])) { // any track incidating?
						...
						if (!MaybeAddToTodoSet(newtile, newdir, index, dir)) return flags | SF_FULL;
					}
				}
		...
		if (!MaybeAddToTodoSet(index, enterdir, oldindex, exitdir)) return flags | SF_FULL;
	}

...
public:
	...
	enum Slope {
		...
		SLOPE_W = ::Slope(::BaseSlope::W).base(), ///< The west corner of the tile is raised.
		SLOPE_S = ::Slope(::BaseSlope::S).base(), ///< The south corner of the tile is raised.
		SLOPE_E = ::Slope(::BaseSlope::E).base(), ///< The east corner of the tile is raised.
		SLOPE_N = ::Slope(::BaseSlope::N).base(), ///< The north corner of the tile is raised.
		SLOPE_STEEP = ::Slope(::BaseSlope::Steep).base(), ///< Indicates the slope is steep (The corner opposite of the not-raised corner is raised two times).
...
public:
	 ...
	 * Halftile slopes appear on top of halftile foundations. E.g. the slope you get when building a horizontal railtrack on the top of a BaseSlope::N or SLOPE_STEEP_N.
	 * @param slope The slope to check on.
	 * @pre slope != SLOPE_INVALID.
	 ...
	 *  for example: BaseSlope::N | BaseSlope::W (= SLOPE_NW) will raise the west and the north corner.
	 ...
	 *  for example: BaseSlope::N | BaseSlope::W (= SLOPE_NW) will lower the west and the north corner.
	 * @note The corners will be modified in the order west (first), south, east, north (last).
	 *       Changing one corner might cause another corner to be changed too. So modifying
	 *       multiple corners may result in changing some corners by multiple steps.

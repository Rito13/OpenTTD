...
static void FillIndustryCatchment(const Industry *i, SQInteger radius, BitmapTileArea &bta)
{
	for (TileIndex cur_tile : i->location) {
		if (!::IsTileType(::Tile(cur_tile), MP_INDUSTRY) || ::GetIndustryIndex(::Tile(cur_tile)) != i->index) continue;
		...
		for (int y = -radius; y <= radius; y++) {
			...
			for (int x = -radius; x <= radius; x++) {
				...
				if (::IsTileType(::Tile(tile), MP_INDUSTRY) && ::GetIndustryIndex(::Tile(tile)) == i->index) continue;
				bta.SetTile(tile);
			}
		}
	}
...
ScriptTileList_StationType::ScriptTileList_StationType(StationID station_id, ScriptStation::StationType station_type)
{
	...
	for (TileIndex cur_tile : ta) {
		if (!::IsTileType(::Tile(cur_tile), MP_STATION)) continue;
		if (::GetStationIndex(::Tile(cur_tile)) != station_id) continue;
		if (!station_types.Test(::GetStationType(::Tile(cur_tile)))) continue;
		this->AddTile(cur_tile);
	}
}

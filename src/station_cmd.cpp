...
std::tuple<CommandCost, bool> ClearTile_Station(TileIndex index, Tile &tile, DoCommandFlag flags);
...
static CommandCost CheckFlatLandRailStation(TileIndex tile_cur, TileIndex north_tile, int &allowed_z, DoCommandFlag flags, Axis axis, StationID *station, RailType rt, std::vector<Train *> &affected_vehicles, StationClassID spec_class, uint16_t spec_index, uint8_t plat_len, uint8_t numtracks)
{
	...
	if (station != nullptr && IsTileType(tile_cur, MP_STATION)) {
		if (!IsRailStation(tile_cur)) {
			Tile t = tile_cur;
			return std::get<0>(ClearTile_Station(tile_cur, t, DC_AUTO)); // get error message
		} else {
...
CommandCost CheckFlatLandRoadStop(TileIndex cur_tile, int &allowed_z, DoCommandFlag flags, uint invalid_dirs, bool is_drive_through, StationType station_type, Axis axis, StationID *station, RoadType rt)
{
	...
	if (station != nullptr && IsTileType(cur_tile, MP_STATION)) {
		...
		if (!IsAnyRoadStop(cur_tile)) {
			return std::get<0>(ClearTile_Station(cur_tile, t, DC_AUTO)); // Get error message.
		} else {
...
static bool TileLoop_Station(TileIndex index, Tile &tile)
{
	...
	switch (GetStationType(tile)) {
		case StationType::Airport:
			AirportTileAnimationTrigger(Station::GetByTile(tile), index, AAT_TILELOOP);
			break;
...
static void AnimateTile_Station(TileIndex index, Tile tile)
{
...
static VehicleEnterTileStatus VehicleEnter_Station(Vehicle *v, TileIndex index, Tile tile, int x, int y)
{
	if (v->type == VEH_TRAIN) {
		...
		if (rv->state < RVSB_IN_ROAD_STOP && !IsReversingRoadTrackdir((Trackdir)rv->state) && rv->frame == 0) {
			if (IsStationRoadStop(tile) && rv->IsFrontEngine()) {
				...
				return RoadStop::GetByTile(index, GetRoadStopType(tile))->Enter(rv) ? VETSB_CONTINUE : VETSB_CANNOT_ENTER;
			}
		}
	}
	...
	if (flags & DC_AUTO) {
		switch (GetStationType(tile)) {
			...
			case StationType::Oilrig:
				...
				return {CommandCost(STR_ERROR_GENERIC_OBJECT_IN_THE_WAY), false};
		}
	}
...
static CommandCost TerraformTile_Station(TileIndex index, Tile tile, DoCommandFlag, int z_new, Slope tileh_new)
{

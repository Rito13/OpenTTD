...
static void DrawTile_Station(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (ti->tileh != SLOPE_FLAT && !IsDock(ti->tile)) {
		if (statspec != nullptr && statspec->flags.Test(StationSpecFlag::CustomFoundations)) {
			...
			} else if (statspec->flags.Test(StationSpecFlag::ExtendedFoundations)) {
...
static VehicleEnterTileStates VehicleEnter_Station(Vehicle *v, TileIndex index, Tile tile, int x, int y)
{
	if (v->type == VEH_TRAIN) {
	...
	if (new_owner != INVALID_OWNER) {
		...
		if (IsDriveThroughStopTile(tile)) {
			...
			if (IsRoadWaypoint(tile)) {
				Command<CMD_REMOVE_FROM_ROAD_WAYPOINT>::Do({DoCommandFlag::Execute, DoCommandFlag::Bankrupt}, index, index);
			} else {
				Command<CMD_REMOVE_ROAD_STOP>::Do({DoCommandFlag::Execute, DoCommandFlag::Bankrupt}, index, 1, 1, (GetStationType(tile) == StationType::Truck) ? RoadStopType::Truck : RoadStopType::Bus, false);
			}

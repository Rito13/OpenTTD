...
static CommandCost ClearTile_Industry(TileIndex tile, DoCommandFlags flags)
{
	...
	if ((_current_company != OWNER_WATER && _game_mode != GM_EDITOR &&
			...
			(_current_company == OWNER_WATER &&
				...
				GetIndustryTileSpec(GetIndustryGfx(tile))->slopes_refused.Test(BaseSlope::HalfTile)))) {
...
bool IsSlopeRefused(Slope current, Slope refused)
{
	...
	if (current != SLOPE_FLAT) {
		...
		if (refused.Test(BaseSlope::W) && t.Any(SLOPE_NW)) return true;
		if (refused.Test(BaseSlope::S) && t.Any(SLOPE_NE)) return true;
		if (refused.Test(BaseSlope::E) && t.Any(SLOPE_SW)) return true;
		if (refused.Test(BaseSlope::N) && t.Any(SLOPE_SE)) return true;
	}
...
static CommandCost CheckIfIndustryTilesAreFree(TileIndex tile, const IndustryTileLayout &layout, IndustryType type)
{
	...
	for (const IndustryTileLayoutTile &it : layout) {
		...
		if (gfx == GFX_WATERTILE_SPECIALCHECK) {
			...
			if (!its->slopes_refused.Test(BaseSlope::HalfTile) && ((HasTileWaterClass(cur_tile) && IsTileOnWater(cur_tile)) != ind_behav.Test(IndustryBehaviour::BuiltOnWater))) return CommandCost(STR_ERROR_SITE_UNSUITABLE);
...
static bool CheckIfCanLevelIndustryPlatform(TileIndex tile, DoCommandFlags flags, const IndustryTileLayout &layout)
{
	...
	for (TileIndex tile_walk : ta) {
		...
		if (curh != h) {
			...
			if (std::get<0>(Command<Commands::TerraformLand>::Do(DoCommandFlags{flags}.Reset(DoCommandFlag::Execute), tile_walk, BaseSlope::N, curh <= h)).Failed()) {
				return false;
			}
		}
	}
	...
	if (flags.Test(DoCommandFlag::Execute)) {
		...
		for (TileIndex tile_walk : ta) {
			...
			while (curh != h) {
				...
				Command<Commands::TerraformLand>::Do(flags, tile_walk, BaseSlope::N, curh <= h);
				curh += (curh > h) ? -1 : 1;
			}
		}
	}

 ...
 * @pre tile < Map::Size()
 */
[[debug_inline]] static inline uint TileHeight(const Tile &tile)
{
	assert(tile.IsValid());
	return tile.height();
}
 ...
 * @pre tile.IsValid()
 */
[[debug_inline]] static inline TileType GetTileType(const Tile &tile)
{
	assert(tile.IsValid());
 ...
 */
[[debug_inline]] static inline bool IsTileType(const Tile &tile, TileType type)
{

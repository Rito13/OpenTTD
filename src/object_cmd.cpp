	...
	switch (type) {
		...
		case OBJECT_HQ: {
			...
			if (c->location_of_HQ != INVALID_TILE) {
				...
				cost.AddCost(std::get<0>(ClearTile_Object(c->location_of_HQ, t, flags)));
...
static Foundation GetFoundation_Object(TileIndex, Tile tile, Slope tileh)
{
	ObjectType type = GetObjectType(tile);
	const ObjectSpec *spec = ObjectSpec::Get(type);
	return spec->flags.Test(ObjectFlag::HasNoFoundation) ? FOUNDATION_NONE : FlatteningFoundation(tileh);
}
...
static std::tuple<CommandCost, bool> ClearTile_Object(TileIndex index, Tile &tile, DoCommandFlags flags)
{
	...
	if (_current_company != OWNER_WATER) {
		...
		} else if (CheckTileOwnership(index).Failed()) {
			/* We don't own it!. */
			return {CommandCost(STR_ERROR_OWNED_BY), false};
...
static void AnimateTile_Object(TileIndex index, Tile tile)
{
	AnimateNewObjectTile(index, tile);
}
...
static bool ChangeTileOwner_Object(TileIndex index, Tile &tile, Owner old_owner, Owner new_owner)
{
...
static CommandCost TerraformTile_Object(TileIndex index, Tile tile, DoCommandFlags, int z_new, Slope tileh_new)
{
	ObjectType type = GetObjectType(tile);
